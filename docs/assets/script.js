...
function formatBeijingDate(date, options) {
    return date.toLocaleString('zh-CN', { timeZone: BEIJING_TIME_ZONE, ...options });
}
...
function formatSaveTime(isoString) {
    ...
    return formatBeijingDate(date, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
}
...
function updateSaveTimeDisplay() {
    ...
    if (configTimeEl) {
        ...
        configTimeEl.title = configTime ? formatBeijingDate(new Date(configTime)) : '未保存';
        if (configLabelEl) {
            if (configTime) {
    ...
    if (frequencyTimeEl) {
        ...
        frequencyTimeEl.title = frequencyTime ? formatBeijingDate(new Date(frequencyTime)) : '未保存';
        if (frequencyLabelEl) {
            if (frequencyTime) {
    ...
    if (timelineTimeEl) {
        ...
        timelineTimeEl.title = timelineTime ? formatBeijingDate(new Date(timelineTime)) : '未保存';
        if (timelineLabelEl) {
            if (timelineTime) {
...
function renderWeekView(config, presetName) {
    ...
    // 获取当前星期几 (1=周一...7=周日)
    const todayIso = getBeijingParts().weekday;
    ...
    for (let d = 1; d <= 7; d++) {
        ...
        // 当前时间指示线（仅今天）
        if (isToday) {
            const nowTime = getBeijingParts();
            const nowH = nowTime.hour + nowTime.minute / 60;
            const nowLeftPct = (nowH / 24 * 100).toFixed(2);
            html += `<div class="tl-now-line" style="left:${nowLeftPct}%" title="北京时间 ${String(nowTime.hour).padStart(2,'0')}:${String(nowTime.minute).padStart(2,'0')}"></div>`;
        }
