...
class DateParser:
    ...
    def _now() -> datetime:
        """获取北京时间当前时间。"""
        return get_configured_time("Asia/Shanghai").replace(tzinfo=None)
    ...
    def parse_date_query(date_query: str) -> datetime:
        ...
        if date_query in DateParser.CN_DATE_MAPPING:
            ...
            return DateParser._now() - timedelta(days=days_ago)
        ...
        if cn_days_ago_match:
            ...
            return DateParser._now() - timedelta(days=days)
        ...
        if cn_date_match:
            ...
            if year_str:
                ...
                year = DateParser._now().year
                # 如果月份大于当前月份，说明是去年
                current_month = DateParser._now().month
    ...
    def validate_date_not_future(date: datetime) -> None:
        ...
        """
        if date.date() > DateParser._now().date():
            raise InvalidParameterError(
    ...
    def validate_date_not_too_old(date: datetime, max_days: int = 365) -> None:
        ...
        """
        days_ago = (DateParser._now().date() - date.date()).days
